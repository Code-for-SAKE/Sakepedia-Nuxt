...
export async function getList (type, params, context=undefined) {
  ...
  const get = async () => {
    if (context) {
      return await context.$axios.get(`/api/${type}`, {params: queryParam}).then((res) => {return res}).catch((e) => console.log(e))
    } else {
      return await axios.get(`/api/${type}`, {params: queryParam}).then((res) => {return res}).catch((e) => console.log(e))
    }
  }
