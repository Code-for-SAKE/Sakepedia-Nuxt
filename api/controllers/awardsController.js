...
module.exports.all = function (req, res, next) {
  ...
  Award.paginate(search, {page: req.query.page, limit: req.query.limit}, function(err, result) {
    if(err) {
...
module.exports.show = function(req, res) {
  ...
  Award.findOne({_id: id}).populate('brewery').exec(function(err, data){
      if(err) {
          return res.status(500).json({
...
module.exports.list = function (req, res, next) {
  ...
  Award.find(search).select('name').limit(10).exec(function(err, datas){
    if(err) {
        return res.status(500).json({
