...
module.exports.all = function (req, res, next) {
  ...
  if(sake) [
    search.sake = {_id: sake}
  ]
  if(keyword) {
    search.$or = [
          {sake: {name: new RegExp(keyword, 'i')}},
      ]
  }
  refSake = {path:'sake', select:'name'}
  BreweryYearData.paginate(search, {page: req.query.page, limit: req.query.limit, populate: refSake, sort:{makedBY: -1, modifiedAt: -1} }, async function(err, result) {
    if(err) {
...
module.exports.list = function (req, res, next) {
  ...
  BreweryYearData.find(search).select('name').limit(10).exec(function(err, data){
    if(err) {
        return res.status(500).json({
...
module.exports.create = [
  ...
  validator.body('makedBY').custom( (value, {req}) => {
    return BreweryYearData.findOne({ makedBY: value, sake: req.body.sake })
      .then( bydata => {
      if (bydata !== null) {
...
module.exports.update = [
  ...
  validator.body('sake', 'Please enter Sake').isLength({ min: 1 }),
  validator.body('makedBY').custom( (value, {req}) => {
    return BreweryYearData.findOne({ makedBY: value, sake: req.body.sake, _id:{ $ne: req.params.id }})
      .then( bydata => {
      if (bydata !== null) {
        return Promise.reject('すでに登録済みです');
      }
  ...
  function(req, res) {
    ...
    BreweryYearData.findOne({_id: id}, function(err, bydata){
        ...
        bydata.aminoAcidContent =  req.body.aminoAcidContent ? req.body.aminoAcidContent : bydata.aminoAcidContent;
        bydata.alcoholContent =  req.body.alcoholContent ? req.body.alcoholContent : bydata.alcoholContent;
        bydata.sakeMeterValue =  req.body.sakeMeterValue ? req.body.sakeMeterValue : bydata.sakeMeterValue;
        bydata.acidity =  req.body.acidity ? req.body.acidity : bydata.acidity;
        bydata.ricePolishingRate =  req.body.ricePolishingRate ? req.body.ricePolishingRate : bydata.ricePolishingRate;
        bydata.sakeYeast =  req.body.sakeYeast ? req.body.sakeYeast : bydata.sakeYeast;
        bydata.riceForMakingKoji =  req.body.riceForMakingKoji ? req.body.riceForMakingKoji : bydata.riceForMakingKoji;
        bydata.sakeRiceExceptForKojiMaking =  req.body.sakeRiceExceptForKojiMaking ? req.body.sakeRiceExceptForKojiMaking : bydata.sakeRiceExceptForKojiMaking;
        bydata.bottledDate =  req.body.bottledDate ? req.body.bottledDate : bydata.bottledDate;
